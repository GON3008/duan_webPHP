...
function createPostAction() {
    ...
    if (empty($name)) {
        ...
        header('Location: /shop.projectmvc.vn/?role=admin&mod=category&action=create');
        die();
    }
    create_category($name, $description);
    push_notification('success', ['Tạo mới danh mục sản phẩm thành công']);
    header('Location: /shop.projectmvc.vn/?role=admin&mod=category');
}
...
function updatePostAction() {
    ...
    if (empty($name)) {
        ...
        header('Location: /shop.projectmvc.vn/?role=admin&mod=category&action=update&id_cate='.$id);
    }
